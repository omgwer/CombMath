...
Функция determinant вычисляет определитель матрицы laplacian с помощью рекурсивного алгоритма.
1) В начале функции проверяется базовый случай. Если размер матрицы graphNodeCountMinisOne равен 1, то это означает, что матрица имеет размер 1x1,
и мы просто возвращаем единственный элемент этой матрицы как определитель.
2) Затем проверяется случай, когда размер матрицы graphNodeCountMinisOne равен 2. В этом случае мы используем формулу для вычисления определителя матрицы 2x2.
3) В остальных случаях (когда размер матрицы больше 2), мы создаем подматрицу submat, которая является матрицей размера (graphNodeCountMinisOne - 1) x (graphNodeCountMinisOne - 1).
...
4) Для каждого столбца k мы вычисляем минор laplacian[0][k] (первый элемент первой строки и k-ый столбец) и умножаем его на соответствующий алгебраический дополнительный знак (k % 2 == 0 ? 1 : -1).
...
5) В конце функция возвращает вычисленное значение определителя det.
Таким образом, рекурсивно разбивая матрицу на подматрицы меньшего размера, алгоритм вычисляет определитель матрицы Лапласа графа, что позволяет найти количество остовных деревьев в графе.
*/
int determinant(vector<vector<int>>& laplacian, int graphNodeCountMinisOne) {
    ...
    } else if (graphNodeCountMinisOne == 2) {
